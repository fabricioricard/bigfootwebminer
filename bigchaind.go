...
import (
	...
	"github.com/bigchain/bigchaind/limits"
...
// winServiceMain is only invoked on Windows.  It detects when bigchaind is running
// as a service and reacts accordingly.
var winServiceMain func() (bool, er.R)

// bigchaindMain is the real main function for bigchaind.  It is necessary to work around
// the fact that deferred functions do not run when os.Exit() is called.  The
// optional serverChan parameter is mainly used by the service code to be
// notified with the server once it is setup so it can gracefully stop it when
// requested from the service control manager.
func bigchaindMain(serverChan chan<- *server) er.R {
	...
	// Perform upgrades to bigchaind as new versions require it.
...
func main() {
	version.SetUserAgentName("bigchaind")
	...
	if err := bigchaindMain(nil); err != nil {
		os.Exit(1)
	}
}
