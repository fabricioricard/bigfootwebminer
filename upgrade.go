...
// oldBtcdHomeDir returns the OS specific home directory bigchaind used prior to
...
func oldBtcdHomeDir() string {
	...
	if appData != "" {
		return filepath.Join(appData, "bigchaind")
	}

	// Fall back to standard HOME directory that works for most POSIX OSes.
	home := os.Getenv("HOME")
	if home != "" {
		return filepath.Join(home, ".bigchaind")
	}
...
// location prior to bigchaind version 0.2.0 and uses heuristics to ascertain the old
...
// upgradeDBPaths moves the databases from their locations prior to bigchaind
...
func upgradeDBPaths() er.R {
	...
	upgradeDBPathNet(filepath.Join(oldDbRoot, "bigchaind.db"), "mainnet")
	upgradeDBPathNet(filepath.Join(oldDbRoot, "bigchaind_testnet.db"), "testnet")
	upgradeDBPathNet(filepath.Join(oldDbRoot, "bigchaind_regtest.db"), "regtest")

	// Remove the old db directory.
	return er.E(os.RemoveAll(oldDbRoot))
}

// upgradeDataPaths moves the application data from its location prior to bigchaind
...
func upgradeDataPaths() er.R {
	...
	if fileExists(oldHomePath) && !fileExists(newHomePath) {
		...
		// Move old bigchaind.conf into new location if needed.
...
// doUpgrades performs upgrades to bigchaind as new versions require it.
