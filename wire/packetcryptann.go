...
// BigCryptAnn is the in-memory structure of a BigCrypt announcement
type BigCryptAnn struct {
	Header [1024]byte
}

// BcAnnSerializeSize how big the announcement is when serialized
const BcAnnSerializeSize = 1024

// BcAnnHeaderLen is the length of the announcement header (not including merkle proof)
const BcAnnHeaderLen = 88

// BcAnnMerkleProofLen is the length of the merkle proof
const BcAnnMerkleProofLen = 896

// PcItem4PrefixLen is the length of the item 4 prefix which follows the merkle proof
const PcItem4PrefixLen = BcAnnSerializeSize - (BcAnnHeaderLen + BcAnnMerkleProofLen)

// GetVersion gets the version number of the announcement.
func (p *BigCryptAnn) GetVersion() uint {
	return uint(p.Header[0])
}

// GetAnnounceHeader provides the header without the merkle proof
func (p *BigCryptAnn) GetAnnounceHeader() []byte {
	return p.Header[:BcAnnHeaderLen]
}

// GetMerkleProof provides the memory location in the announcement of the merkle branch
func (p *BigCryptAnn) GetMerkleProof() []byte {
	return p.Header[BcAnnHeaderLen : BcAnnHeaderLen+BcAnnMerkleProofLen]
}

// GetItem4Prefix provides the memory location of item4 which must be stored in the announcement
func (p *BigCryptAnn) GetItem4Prefix() []byte {
	return p.Header[BcAnnHeaderLen+BcAnnMerkleProofLen:]
}

// GetSoftNonce outputs a byte slice with bytes 1, 2 and 3
func (p *BigCryptAnn) GetSoftNonce() []byte {
	return p.Header[1:4]
}
...
func (p *BigCryptAnn) GetParentBlockHeight() uint32 {
...
func (p *BigCryptAnn) GetContentHash() []byte {
