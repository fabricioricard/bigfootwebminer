...
import (
	...
	"github.com/bigchain/bigchaind/blockchain/testdata"
...
func TestInsertCoinbaseCommit(t *testing.T) {
	...
	cbc := wire.BcCoinbaseCommit{}
	copy(cbc.Bytes[:], cbcBytes)

	block := testdata.GetBlock("../testdata/277647.dat.bz2", t)
	mb := block.MsgBlock()

	bigcrypt.InsertCoinbaseCommit(mb.Transactions[0], &cbc)

	cbc2 := bigcrypt.ExtractCoinbaseCommit(mb.Transactions[0])
	if cbc2 == nil {
