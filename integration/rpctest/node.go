...
import (
	...
	rpc "github.com/bigchain/bigchaind/rpcclient"
)

// nodeConfig contains all the args, and data required to launch a bigchaind process
...
func newConfig(prefix, certFile, keyFile string, extra []string) (*nodeConfig, er.R) {
	bigchaindPath, err := bigchaindExecutablePath()
	if err != nil {
		bigchaindPath = "bigchaind"
	}
...
// arguments returns an array of arguments that be used to launch the bigchaind
// process.
...
// command returns the exec.Cmd which will be used to start the bigchaind process.
...
// to the bigchaind process that is launched via Start().
...
// as the base for the log and data directories for bigchaind.
...
// start creates a new bigchaind process, and writes its pid in a file reserved for
...
func (n *node) start() er.R {
	fmt.Printf("Launching bigchaind with arguments: %v\n", n.cmd.Args)
...
// stop interrupts the running bigchaind process process, and waits until it exits
...
// shutdown terminates the running bigchaind process, and cleans up all
