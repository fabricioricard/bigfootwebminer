...
var (
	bigchaindHomeDir           = btcutil.AppDataDir("bigchaind", false)
	btcctlHomeDir         = btcutil.AppDataDir("btcctl", false)
	bigchainwalletHomeDir      = btcutil.AppDataDir("bigchainwallet", false)
	defaultConfigFile     = filepath.Join(btcctlHomeDir, "btcctl.conf")
	defaultRPCServer      = "localhost"
	defaultRPCCertFile    = filepath.Join(bigchaindHomeDir, "rpc.cert")
	defaultWalletCertFile = filepath.Join(bigchainwalletHomeDir, "rpc.cert")
)
...
func loadConfig() (*config, []string, er.R) {
	...
	if preCfg.Wallet {
		serverConfigPath = filepath.Join(bigchainwalletHomeDir, "bigchainwallet.conf")
	} else {
		serverConfigPath = filepath.Join(bigchaindHomeDir, "bigchaind.conf")
	}

	if userpass, err := bigchainconfig.ReadUserPass(serverConfigPath); err != nil {
		fmt.Fprintf(os.Stderr, "Warning: cannot open file [%s] [%s]\n",
			serverConfigPath, err.String())
	} else if len(userpass) != 2 {
		dir := bigchaindHomeDir
		if preCfg.Wallet {
			dir = bigchainwalletHomeDir
		}
		if cfg.RPCPassword != "" {
