...
const (
	// svcName is the name of bigchaind service.
	svcName = "bigchaindsvc"
	...
	svcDesc = "Synchronizes with the BigChain blockchain" +
...
// logServiceStartOfDay logs information about bigchaind when the main server has
...
// bigchaindService houses the main service handler which handles all service
// updates and launching bigchaindMain.
type bigchaindService struct{}
...
// long-running bigchaindMain (which is the real meat of bigchaind), handles service
// change requests, and notifies the service control manager of changes.
func (s *bigchaindService) Execute(args []string, r <-chan svc.ChangeRequest, changes chan<- svc.Status) (bool, uint32) {
	// Service start is pending.
	const cmdsAccepted = svc.AcceptStop | svc.AcceptShutdown
	changes <- svc.Status{State: svc.StartPending}

	// Start bigchaindMain in a separate goroutine so the service can start
...
// installService attempts to install the bigchaind service.  Typically this should
...
// removeService attempts to uninstall the bigchaind service.  Typically this should
...
// startService attempts to start the bigchaind service.
...
func serviceMain() (bool, er.R) {
	...
	errr = svc.Run(svcName, &bigchaindService{})
	if errr != nil {
