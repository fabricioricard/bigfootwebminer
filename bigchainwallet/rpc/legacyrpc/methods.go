...
// information about the current state of bigchainwallet.
// exist.
func getInfo(icmd interface{}, w *wallet.Wallet, chainClient chain.Interface) (interface{}, er.R) {
	...
	if rpc, ok := chainClient.(*chain.RPCClient); ok {
		// Call down to bigchaind for all of the information in this command known
		// by them.
...
func help(icmd interface{}, w *wallet.Wallet, chainClient *chain.RPCClient) (interface{}, er.R) {
	...
	// bigchaind returns different help messages depending on the kind of
	...
	// wallet itself is a websocket client to bigchaind.  Therefore, create a
	...
	// texts in a non-internal bigchaind package.
...
func sendTxRequest(w *wallet.Wallet, req *wallet.CreateTxReq) (*txauthor.AuthoredTx, er.R) {
	...
	if err != nil {
		...
		if waddrmgr.ErrLocked.Is(err) {
			return nil, er.New("Enter the wallet passphrase with `./bin/bigchainctl --wallet walletpassphrase` first")
		}
		if btcjson.Err.Is(err) {
			return nil, err
		}
...
func signRawTransaction(icmd interface{}, w *wallet.Wallet, chainClient chain.Interface) (interface{}, er.R) {
	...
	// querying bigchaind with getrawtransaction. We queue up a bunch of async
	...
	for _, txIn := range tx.TxIn {
		...
		if !ok {
			return nil, er.New("You must specify all transaction inputs explicitly, " +
				"or use --userpc to load them from bigchaind.")
		}
