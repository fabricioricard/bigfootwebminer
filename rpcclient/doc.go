...
Bitcoin RPC server that uses a bigchaind/bitcoin core compatible Bitcoin JSON-RPC
API.  This client has been tested with bigchaind (https://github.com/bigchain/bigchaind),
bigchainwallet (https://github.com/bigchain/bigchainwallet), and
bitcoin core (https://github.com/bitcoin).

In addition to the compatible standard HTTP POST JSON-RPC API, bigchaind and
bigchainwallet provide a websocket interface that is more efficient than the standard
...
bigchaind or bigchainwallet by default.  However, configuration options are provided to
...
In contrast, the websocket-based JSON-RPC interface provided by bigchaind and
bigchainwallet only uses a single connection that remains open and allows
...
All notifications provided by bigchaind require registration to opt-in.  For example,
...
example, the DebugLevel call is an extension only provided by bigchaind (and
bigchainwallet passthrough).  Therefore if you call one of these commands against
...
Also, it is important to realize that bigchaind intentionally separates the wallet
functionality into a separate process named bigchainwallet.  This means if you are
connected to the bigchaind RPC server directly, only the RPCs which are related to
chain services will be available.  Depending on your application, you might only
need chain-related RPCs.  In contrast, bigchainwallet provides pass through treatment
...
The following full-blown client examples are in the examples directory:
 ...
 - bigchaindwebsockets
   Connects to a bigchaind RPC server using TLS-secured websockets, registers for
   block connected and block disconnected notifications, and gets the current
   block count
 - bigchainwalletwebsockets
   Connects to a bigchainwallet RPC server using TLS-secured websockets, registers
