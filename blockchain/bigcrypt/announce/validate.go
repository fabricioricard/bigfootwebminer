...
import (
	...
	"github.com/bigchain/bigchaind/blockchain/bigcrypt/randhash/interpret"
	"github.com/bigchain/bigchaind/blockchain/bigcrypt/randhash/randgen"
...
func annDecrypt(pcAnn *wire.BigCryptAnn, state *cryptocycle.State) *wire.BigCryptAnn {
	out := wire.BigCryptAnn{}
	copy(out.Header[:], pcAnn.Header[:])
	j := 0
	for i := wire.BcAnnHeaderLen; i < wire.BcAnnHeaderLen+wire.BcAnnMerkleProofLen-64; i++ {
		out.Header[i] ^= state.Bytes[j]
		j++
	}
	for i := wire.BcAnnHeaderLen + wire.BcAnnMerkleProofLen; i < 1024; i++ {
		out.Header[i] ^= state.Bytes[j]
		j++
	}
	return &out
...
func CheckAnn(pcAnn *wire.BigCryptAnn, parentBlockHash *chainhash.Hash, packetCryptVersion int) (*chainhash.Hash, er.R) {
	...
	pcutil.HashCompress64(ctx.annHash0[:], ctx.ann.Header[:wire.BcAnnHeaderLen+64])
	copy(ctx.ann.GetMerkleProof(), pcAnn.GetMerkleProof()[13*64:])
	pcutil.HashCompress64(ctx.annHash1[:], ctx.ann.Header[:wire.BcAnnHeaderLen+64])
