...
import (
	...
	"github.com/bigchain/bigchaind/bigchainconfig"
	"github.com/bigchain/bigchaind/bigchainconfig/version"
	"github.com/bigchain/bigchaind/bigchainlog/log"
	"github.com/bigchain/bigchaind/bigchainwallet/internal/cfgutil"
	"github.com/bigchain/bigchaind/bigchainwallet/internal/legacy/keystore"
	"github.com/bigchain/bigchaind/bigchainwallet/netparams"
	"github.com/bigchain/bigchaind/bigchainwallet/wallet"
)

const (
	defaultCAFilename       = "bigchaind.cert"
	defaultConfigFilename   = "bigchainwallet.conf"
...
var (
	bigchaindDefaultCAFile  = filepath.Join(btcutil.AppDataDir("bigchaind", false), "rpc.cert")
	bigchaindDefaultConf    = filepath.Join(btcutil.AppDataDir("bigchaind", false), "bigchaind.conf")
	defaultAppDataDir  = btcutil.AppDataDir("bigchainwallet", false)
...
type config struct {
	...
	Wallet        string                  `short:"w" long:"wallet" description:"Wallet file name or path, if a simple word such as 'personal' then bigchainwallet will look for wallet_personal.db, if prefixed with a / then bigchainwallet will consider it an absolute path."`
	...
	// RPC client options
	RPCConnect       string                  `short:"c" long:"rpcconnect" description:"Hostname/IP and port of bigchaind RPC server to connect to (default localhost:8334, testnet: localhost:18334, simnet: localhost:18556)"`
	CAFile           *cfgutil.ExplicitString `long:"cafile" description:"File containing root certificates to authenticate a TLS connections with bigchaind"`
	DisableClientTLS bool                    `long:"noclienttls" description:"nolonger used" hidden:"true"`
	ClientTLS        bool                    `long:"clienttls" description:"enable tls to the bigchaind instance"`
	BtcdUsername     string                  `long:"bigchaindusername" description:"Username for bigchaind authentication"`
	BtcdPassword     string                  `long:"bigchaindpassword" default-mask:"-" description:"Password for bigchaind authentication"`
	...
	UseRPC                 bool                    `long:"userpc" description:"Use an RPC connection to bigchaind rather than using neutrino, the default behavior is to connect to a single local bigchaind instance using neutrino, UseSPV will make neutrino connect to multiple nodes"`
	...
	Username               string                  `short:"u" long:"rpcuser" description:"Username for legacy RPC and bigchaind authentication (if bigchaindusername is unset)"`
	Password               string                  `short:"P" long:"rpcpass" default-mask:"-" description:"Password for legacy RPC and bigchaind authentication (if bigchaindpassword is unset)"`
...
//
// The above results in bigchainwallet functioning properly without any config
...
func loadConfig() (*config, []string, er.R) {
	...
	// Attempt to grab the user/pass from bigchaind.conf
	...
	} else if userpass, err := bigchainconfig.ReadUserPass(bigchaindDefaultConf); err != nil {
	...
	if errr := flags.NewIniParser(parser).ParseFile(configFilePath); errr != nil {
		...
		if configFileError = bigchainconfig.CreateDefaultConfigFile(
			configFilePath, bigchainconfig.PktwalletSampleConfig); configFileError != nil {
		} else {
	...
	if !cfg.UseRPC {
		...
		if cfg.ClientTLS {
			// If CAFile is unset, choose either the copy or local bigchaind cert.
			if !cfg.CAFile.ExplicitlySet() {
				cfg.CAFile.Value = filepath.Join(cfg.AppDataDir.Value, defaultCAFilename)

				// If the CA copy does not exist, check if we're connecting to
				// a local bigchaind and switch to its RPC cert if it exists.
				...
				if !certExists {
					if _, ok := localhostListeners[RPCHost]; ok {
						bigchaindCertExists, err := cfgutil.FileExists(
							bigchaindDefaultCAFile)
						if err != nil {
							fmt.Fprintln(os.Stderr, err)
							return nil, nil, err
						}
						if bigchaindCertExists {
							cfg.CAFile.Value = bigchaindDefaultCAFile
						}
					}
				}
			}
	...
	// If the bigchaind username or password are unset, use the same auth as for
	// the client.  The two settings were previously shared for bigchaind and
	// client auth, so this avoids breaking backwards compatibility while
	// allowing users to use different auth settings for bigchaind and wallet.
	if cfg.BtcdUsername == "" {
